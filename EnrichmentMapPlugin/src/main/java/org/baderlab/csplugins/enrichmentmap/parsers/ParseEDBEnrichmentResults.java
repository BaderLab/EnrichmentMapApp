	...
	private class EDBHandler extends DefaultHandler {
		...
		public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
			if ("DTG".equals(qName)) {
				...
				// gsSize - geneset size. Get value from the number of hits in the hit indices
				// (HIT_INDICES)
				...
				if(nesFilter == NESFilter.ALL || 
					(nesFilter == NESFilter.NEGATIVE && NES < 0) ||
					(nesFilter == NESFilter.POSITIVE && NES >= 0)) {
					
					GSEAResult result = new GSEAResult(name, gsSize, ES, NES, pvalue, FDR, FWER, (int) rankAtMax,
							scoreAtMax);
