...
public class LoadSignatureGMTFilesTask implements TaskFactory {
	...
	public LoadSignatureGMTFilesTask(EnrichmentMap map, PostAnalysisParameters paParams, StreamUtil streamUtil,
			PostAnalysisInputPanel paPanel) {
	...
	public void load() {
		...
		try {
			...
			GMTFileReaderTask gmtFile_2 = new GMTFileReaderTask(map, paParams, GMTFileReaderTask.SIGNATURE_GMT,
					streamUtil);
			loadSigGMTTaskIterator.append(gmtFile_2);

			//filter signature genesets
			FilterSignatureGSTask filterSigGs = new FilterSignatureGSTask(map, paParams, paPanel);
			loadSigGMTTaskIterator.append(filterSigGs);
